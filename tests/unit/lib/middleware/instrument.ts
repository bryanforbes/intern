		...
		tests: {
			'instrumented file': {
				...
				tests: {
					...
					'read error'() {
						sandbox.stub(fs, 'stat').callsFake((path: string, callback: any) => {
							const data = fs.__fileData['/base/foo/thing.js'];
			...
			'non-instrumented file'() {
				...
				assert.lengthOf(next.firstCall.args, 0, 'next should have been called with no arguments');
			},
